...
export function _activeRunsForTest(): Map<string, ActiveRun> {
  return activeRuns;
}
...
const benchmarkRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post("/benchmarks/runs", async (request, reply) => {
    ...
    const entry: ActiveRun = { runId, startedAt, args: built.args, pid: null, exitCode: null, status: "running", output: "" };
      ...
      const append = async (stream: ReadableStream<Uint8Array> | null | undefined) => {
        ...
        for (;;) {
          ...
          if (entry.output.length < MAX_OUTPUT_BYTES) {
            entry.output += dec.decode(value);
            if (entry.output.length > MAX_OUTPUT_BYTES) entry.output = entry.output.slice(0, MAX_OUTPUT_BYTES);
          }
        }
      };
