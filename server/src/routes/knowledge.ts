...
const knowledgeRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/projects/:projectId/knowledge/stats", async (request) => {
    ...
    const artifactCount = (
      db
        .prepare("SELECT COUNT(*) as n FROM project_artifacts WHERE projectId = ?")
    ...
    const chunkCount = (
      db
        .prepare("SELECT COUNT(*) as n FROM artifact_embeddings WHERE projectId = ?")
        .get(projectId) as any
    ).n as number;

    const taskCount = (
      db
        .prepare("SELECT COUNT(*) as n FROM tasks WHERE projectId = ? AND status != 'archived'")
        .get(projectId) as any
    ).n as number;

    const embeddedTasks = (
      db
        .prepare("SELECT COUNT(DISTINCT taskId) as n FROM task_embeddings WHERE projectId = ?")
        .get(projectId) as any
    ).n as number;

    const taskChunkCount = (
      db
        .prepare("SELECT COUNT(*) as n FROM task_embeddings WHERE projectId = ?")
        .get(projectId) as any
    ).n as number;

    const graphNodeCount = (
      db
        .prepare("SELECT COUNT(*) as n FROM project_graph_nodes WHERE projectId = ?")
        .get(projectId) as any
    ).n as number;

    const embeddedGraphNodes = (
      db
        .prepare(
    ...
    const graphNodeChunkCount = (
      db
        .prepare("SELECT COUNT(*) as n FROM graph_node_embeddings WHERE projectId = ?")
        .get(projectId) as any
    ).n as number;
