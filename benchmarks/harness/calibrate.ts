...
export function calibrate(
  ...
  const harderThreshold = opts.harderThreshold ?? 0.2;
  ...
  for (const [fixtureId, results] of byFixture) {
    ...
    const avgDurationMs =
      total === 0 ? 0 : results.reduce((s, r) => s + (r.durationMs ?? 0), 0) / total;
  ...
  return {
    ...
    resultsScanned: filtered.reduce(
...
export function formatCalibrationText(c: CalibrationReport): string {
  const lines: string[] = [];
  lines.push(
    `Benchmark calibration · window ${c.windowDays}d · ${c.resultsScanned} results across ${c.reportsScanned} runs`,
  );
  lines.push(
    `Thresholds: ≥ ${pct(c.trivialThreshold)} trivial · ≤ ${pct(c.harderThreshold)} harder · min ${c.minSamples} samples`,
  );
  lines.push("");
  ...
  if (trivial.length > 0) {
    ...
    for (const f of trivial) {
      lines.push(
        `  ${f.fixtureId} [${f.difficulty}] ${pct(f.solveRate)} (${f.solved}/${f.total}) · ${fmtTurns(f.avgTurns)} turns · ${fmtCost(f.avgCostUsd)}`,
      );
    }
    lines.push("");
  }
  ...
  if (insufficient.length > 0) {
    lines.push(
      `Need more samples (${insufficient.length}): ${insufficient.map((f) => f.fixtureId).join(", ")}`,
    );
    lines.push("");
  }
  if (meta.length > 0) {
    lines.push(
      `Harness self-tests (${meta.length}, not graded): ${meta.map((f) => f.fixtureId).join(", ")}`,
    );
    lines.push("");
  }
  ...
  for (const f of c.fixtures) {
    const flag =
      f.recommendation === "trivial"
        ? "trivial"
        : f.recommendation === "harder"
          ? "harder "
          : f.recommendation === "insufficient"
            ? "n<min  "
            : f.recommendation === "meta"
              ? "meta   "
              : "ok     ";
    lines.push(
      `  ${f.fixtureId.padEnd(36)} ${f.difficulty.padEnd(6)} ${String(f.total).padStart(3)} ${pct(f.solveRate).padStart(7)} ${fmtTurns(f.avgTurns).padStart(7)} ${fmtCost(f.avgCostUsd).padStart(9)}  ${flag}`,
    );
  }
  return lines.join("\n");
}
...
export function formatCalibrationMd(c: CalibrationReport): string {
  ...
  lines.push("");
  lines.push(
    `Window: **${c.windowDays}d** · scanned **${c.resultsScanned}** results across **${c.reportsScanned}** runs`,
  );
  lines.push(
    `Thresholds: solveRate ≥ **${pct(c.trivialThreshold)}** → trivial · ≤ **${pct(c.harderThreshold)}** → harder · min **${c.minSamples}** samples`,
  );
  lines.push("");
  ...
  for (const s of sections) {
    ...
    for (const f of items) {
      lines.push(
        `- **${f.fixtureId}** [${f.difficulty}] — ${pct(f.solveRate)} (${f.solved}/${f.total}) · ${fmtTurns(f.avgTurns)} turns · ${fmtCost(f.avgCostUsd)} — ${f.reason}`,
      );
    }
    lines.push("");
  }
  ...
  for (const f of c.fixtures) {
    lines.push(
      `| ${f.fixtureId} | ${f.difficulty} | ${f.total} | ${pct(f.solveRate)} | ${fmtTurns(f.avgTurns)} | ${fmtCost(f.avgCostUsd)} | ${f.recommendation} |`,
    );
  }
  lines.push("");
  return lines.join("\n");
