...
export function useProjectStats(projectId: string | undefined) {
  return useQuery({
    ...
    queryFn: async (): Promise<ProjectStats> => {
      ...
      const [backlog, todo, ip, done, approved, gitStatus] = await Promise.allSettled([
        ...
        api.tasks.list(projectId!, { status: "approved" as any, limit: 1 }),
      ...
      const approvedTotal = approved.status === "fulfilled" ? approved.value.total : 0;
