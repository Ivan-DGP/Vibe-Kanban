...
function FileContextMenu({
  ...
  const handleRename = () => {
    ...
    renameFile.mutate(
      { projectId, oldPath: filePath, newPath: target },
      {
  ...
  const handleCreate = () => {
    ...
    createFile.mutate(
      { projectId, filePath: target, type: creating },
      {
        onSuccess: () => {
          setCreating(null);
          setCreateName("");
  ...
  const handleDelete = async () => {
    if (!(await confirmDialog({ title: "Delete File", description: `Delete "${currentName}"?` })))
      return;
        ...
        children?.map((entry) =>
          entry.type === "directory" ? (
            ...
            <FileContextMenu key={entry.path} projectId={projectId} path={entry.path} isDir={false}>
              ...
              >
                {(() => {
                  const I = getIcon(entry.name, false, false);
                  return <I className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />;
                })()}
