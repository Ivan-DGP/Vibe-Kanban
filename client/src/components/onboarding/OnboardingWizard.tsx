        ...
        {step === 0 && (
          <OnboardingStep step={0} totalSteps={3} title="Welcome to Vibe Kanban!">
            ...
            <p className="text-sm text-muted-foreground">
              Your local development dashboard for managing projects, tasks, git, terminals, and AI
              — all in one place.
            </p>
            ...
            {scanned.length > 0 && (
              <ScrollArea className="h-[150px] border rounded-md p-2 mt-2">
                {scanned.map((p, i) => (
                  <label
                    key={i}
                    className="flex items-start gap-2 p-1 rounded hover:bg-accent cursor-pointer"
                  >
                    <Checkbox
          ...
          <OnboardingStep step={2} totalSteps={3} title="You're Ready!">
            <div className="space-y-3 text-sm text-muted-foreground">
              <div className="flex items-start gap-2">
                ...
                <span>
                  <strong>Ctrl+K</strong> — Command Palette for quick navigation
                </span>
              </div>
              <div className="flex items-start gap-2">
                <FolderSearch className="h-4 w-4 mt-0.5 shrink-0" />
                <span>
                  <strong>Ctrl+Shift+F</strong> — Global search across projects and tasks
                </span>
              </div>
              <div className="flex items-start gap-2">
                <Sparkles className="h-4 w-4 mt-0.5 shrink-0" />
                <span>
                  Use <strong>AI Resolve</strong> on tasks to let Claude implement them
                </span>
              </div>
            </div>
            <Button className="w-full mt-6" onClick={handleFinish}>
              Start Using Vibe Kanban
            </Button>
          </OnboardingStep>
        )}
      </DialogContent>
