...
test("Vibe Kanban full walkthrough", async ({ page }) => {
  ...
  const titleInput = page
    .getByPlaceholder(/title|task name|task title/i)
  ...
  if (priorityVisible) {
    const tagName = await priorityLabel
    ...
    if (tagName === "select") {
      ...
      const urgentOpt = page
        .getByRole("option", { name: /urgent/i })
        .or(page.getByText(/^urgent$/i));
    ...
    if (await urgentOpt.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const urgentEl = page
        .locator('[value="urgent"], [data-value="urgent"]')
        .or(page.getByText(/^urgent$/i));
