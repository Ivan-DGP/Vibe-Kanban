...
test('AI Resolve - Bun.spawn fix: first output frame arrives within 60s', async ({ page }) => {
  ...
  if (await getStartedBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    if (await startBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      await startBtn.click(); await page.waitForTimeout(1000);
    }
  ...
  await page.waitForFunction(() =>
    document.body.innerText.includes('Bug fix') ||
    document.body.innerText.includes('Backlog'),
    { timeout: 10000 }
  ...
  console.log(`First output at:                       ${firstOutputAt >= 0 ? firstOutputAt + 'ms' : 'never'}`);
  ...
  expect(frames.length, `Expected more than 2 WS frames, got ${frames.length}. The server may have crashed on the new Bun.spawn code.`).toBeGreaterThan(2);

  // 2. At least one output frame must have arrived.
  //    This is the core assertion: Claude's output streams through.
  expect(outputFrames.length, `Expected at least 1 output frame. Bun.spawn stdout pipe may not be working.`).toBeGreaterThan(0);
});
