...
test('AI Resolve retest - verify command appears in terminal', async ({ page }) => {
  ...
  if (await testProjectEl.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    await page.goto(`${BASE_URL}/projects/81268928-97cd-4923-bce1-431237620977`, { waitUntil: 'networkidle' });
    await page.waitForTimeout(2000);
  }
  ...
  await page.waitForFunction(() => {
    ...
    return body.includes('Bug fix') || body.includes('Test kanban task') || body.includes('Inbox');
  }, { timeout: 10000 }).catch(() => console.log('Timeout waiting for task cards'));
  ...
  if (aiCount === 0) {
    ...
    for (const btn of allBtns) {
      ...
      if (combined.includes('resolve') || combined.includes('zap') || combined.includes('ai resolve')) {
  ...
  const allOutput = outputFrames.map((f: WsFrame) => {
  ...
  // Also check DOM
  const xtermRows = await page.locator('.xterm-rows').first().innerText().catch(() => null);
  const xtermA11y = await page.locator('.xterm-accessibility-tree').first().innerText().catch(() => null);
  console.log('\n=== xterm-rows text ===');
  ...
  console.log(`Contains "vk-resolve":                 ${allText.toLowerCase().includes('vk-resolve')}`);
  console.log(`Contains "--dangerously-skip-permissions": ${allText.toLowerCase().includes('dangerously')}`);
