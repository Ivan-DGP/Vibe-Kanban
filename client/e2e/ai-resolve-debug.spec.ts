...
test.describe("AI Resolve - Full Diagnostic", () => {
  test("click AI Resolve and trace WS messages + terminal output", async ({ page }) => {
      ...
      .waitForFunction(
        () => {
          ...
          return (
            ...
            body.includes("Test kanban task") ||
            body.includes("Backlog")
          );
        },
        { timeout: 10000 },
      )
      .catch(() => console.log("Task wait timed out"));
    ...
    console.log(
      "Visible buttons after hover:",
      visButtons.filter(Boolean).map((b: string) => b.trim()),
    );
    ...
    const createMsg = frames
      .filter((f) => f.dir === "out")
      .find((f) => {
        try {
          return JSON.parse(f.data).type === "create";
        } catch {
          return false;
        }
      });
    ...
    console.log(
      "WS CLOSE:",
      closeFrames.length,
      closeFrames.map((f) => `${f.data}@+${f.t - t0}ms`),
    );
    ...
    if (!createMsg) {
      ...
      if (timeSinceCreate < 2000) {
        ...
        console.log(
          "  Server onclose kills all sessions => launchAiResolve finds session gone => no command written",
        );
      }
    } else if (outputFrames.length === 0) {
      console.log(
        "BUG: create sent, WS open, but no output - server crashed writing PTY (ptyProcess.write threw)",
      );
    } else {
      const combined = outputFrames
        .map((f: WsFrame) => {
          try {
