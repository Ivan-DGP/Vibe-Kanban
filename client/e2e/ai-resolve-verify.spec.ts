...
test('AI Resolve button opens terminal with claude command', async ({ page }) => {
  ...
  await page.screenshot({ path: 'client/e2e/screenshots/ai-resolve-01-mounted.png', fullPage: true });

  await page.waitForTimeout(2000);
  await page.screenshot({ path: 'client/e2e/screenshots/ai-resolve-02-settled.png', fullPage: true });
  ...
  await page.screenshot({ path: 'client/e2e/screenshots/ai-resolve-03-card-visible.png', fullPage: true });
  ...
  for (const btn of buttons) {
    ...
    if (combined.includes('resolve') || (combined.includes('zap') && !combined.includes('zapier'))) {
  ...
  if (!aiResolveBtn) {
    ...
    if (box) {
      ...
      await page.screenshot({ path: 'client/e2e/screenshots/ai-resolve-04b-right-edge.png', fullPage: true });
    }
    ...
    for (const btn of allBtns) {
      ...
      const html = await btn.innerHTML().catch(() => '');
      const combined = (title + ' ' + ariaLabel + ' ' + html).toLowerCase();
      console.log(`  btn title="${title}" aria="${ariaLabel}" html[100]="${html.substring(0,100)}"`);
      if (combined.includes('resolve') || (combined.includes('zap') && !combined.includes('zapier'))) {
        aiResolveBtn = btn;
      }
    }
  ...
  await page.screenshot({ path: 'client/e2e/screenshots/ai-resolve-05-clicked.png', fullPage: true });

  console.log('Waiting 5 seconds for terminal...');
  await page.waitForTimeout(5000);

  await page.screenshot({ path: 'client/e2e/screenshots/ai-resolve-06-terminal-5s.png', fullPage: true });
  console.log('Final screenshot saved.');

  const termText = await page.locator('.xterm-rows').first().innerText().catch(() => 'no xterm-rows found');
