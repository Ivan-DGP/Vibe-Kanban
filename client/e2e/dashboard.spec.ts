...
test.describe.serial("Create and delete project", () => {
  ...
  test("Create a new project", async ({ page }) => {
    ...
    await expect(page.locator("h3").filter({ hasText: testProjectName })).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("Delete the test project", async ({ page }) => {
    ...
    await page.waitForURL(/\/$/, { timeout: 10000 });
    await expect(page.locator("h3").filter({ hasText: testProjectName })).not.toBeVisible({
      timeout: 5000,
    });
  });
});
